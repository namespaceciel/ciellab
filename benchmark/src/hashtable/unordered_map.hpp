...
struct sherwood_v10_entry {
    ...
    using EntryPointer = typename std::allocator_traits<
        typename std::allocator_traits<Allocator>::template rebind_alloc<sherwood_v10_entry>>::pointer;
...
template<typename T, typename FindKey, typename ArgumentHash, typename Hasher, typename ArgumentEqual, typename Equal,
         typename ArgumentAlloc, typename EntryAlloc, typename BucketAllocator>
class sherwood_v10_table : private EntryAlloc,
                           private Hasher,
                           private Equal,
...
public:
    ...
    explicit sherwood_v10_table(size_type bucket_count, const ArgumentHash& hash = ArgumentHash(),
    ...
    sherwood_v10_table(std::initializer_list<T> il, size_type bucket_count, const ArgumentHash& hash,
    ...
    sherwood_v10_table(sherwood_v10_table&& other) noexcept
        : EntryAlloc(std::move(other)),
          ...
          BucketAllocator(std::move(other)),
          _max_load_factor(other._max_load_factor) {
        swap_pointers(other);
    }
    ...
    sherwood_v10_table& operator=(const sherwood_v10_table& other) {
        ...
        if (AllocatorTraits::propagate_on_container_copy_assignment::value) {
            ...
            AssignIfTrue<BucketAllocator, BucketAllocatorTraits::propagate_on_container_copy_assignment::value>()(
                *this, other);
        }
        _max_load_factor            = other._max_load_factor;
    ...
    sherwood_v10_table& operator=(sherwood_v10_table&& other) noexcept {
        ...
        if (this == std::addressof(other)) {
            ...
            AssignIfTrue<EntryAlloc, AllocatorTraits::propagate_on_container_move_assignment::value>()(
                *this, std::move(other));
            AssignIfTrue<BucketAllocator, BucketAllocatorTraits::propagate_on_container_move_assignment::value>()(
                *this, std::move(other));
            swap_pointers(other);
...
class unordered_map
    : public detailv10::sherwood_v10_table<
          ...
          typename std::allocator_traits<A>::template rebind_alloc<typename std::allocator_traits<
              A>::template rebind_traits<detailv10::sherwood_v10_entry<std::pair<K, V>, A>>::pointer>> {
...
class unordered_set : public detailv10::sherwood_v10_table<
                          T, T, H, detailv10::functor_storage<size_t, H>, E, detailv10::functor_storage<bool, E>, A,
    ...
    using Table = detailv10::sherwood_v10_table<
        ...
        typename std::allocator_traits<A>::template rebind_alloc<
            typename std::allocator_traits<A>::template rebind_traits<detailv10::sherwood_v10_entry<T, A>>::pointer>>;
