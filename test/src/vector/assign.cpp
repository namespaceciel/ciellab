...
template<class C>
void test_operator_copy_impl(::testing::Test*, C& lhs, C& rhs) {
    rhs = lhs;
    ASSERT_EQ(lhs, rhs);
...
template<class C>
void test_operator_move_impl(::testing::Test*, C& lhs, C& rhs) {
    const auto temp = lhs;
    rhs             = std::move(lhs);
    ASSERT_EQ(temp, rhs);
}
...
void test_assign_iterator_range_impl(::testing::Test*, const C& c) {
...
template<class C>
void test_assign_size_value_impl(::testing::Test*, C& v) {
    using T = typename C::value_type;
